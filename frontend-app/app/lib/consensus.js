/**
 * 🤝 SOMNIA COLLABORATIVE REASONING FORUM (Ported to JS)
...
export class CollaborativeForum {
  constructor() {
    this.sharedContext = [];
    this.weights = { AGEN1: 0.40, AGEN2: 0.35, AGEN3: 0.25 };
  }

  /**
   * Agen masuk ke forum, mendengarkan yang lain, dan memberikan opini berbasis data.
   */
  async participate(agentName, rawData, currentCoin, context = "") {
    let opinion = "";
    ...
    switch(agentName) {
      ...
      case "AGEN1": // SANG TEKNISI (Technical & IL Specialist)
        ...
        if (isDeFi) {
          ...
          opinion = `[TECH] Menganalisa rute LP dari AGEN0. Volatilitas pair ${currentCoin} sedang ${stability}. Risiko IL minimal.`;
        } else {
      ...
      case "AGEN2": // SANG INTEL ON-CHAIN (Whale & TVL Specialist)
        ...
        if (isDeFiFlow) {
          ...
          opinion = `[ON-CHAIN] Deteksi ${flowType} sebesar $${Math.abs(Math.floor(whaleFlow)).toLocaleString()}. Smart money sedang bergerak ${bias > 0 ? "akumulatif" : "distributif"}.`;
        }
        break;
      ...
      case "AGEN3": // SANG FUNDAMENTALIS (Sentiment & Security Specialist)
        ...
        if (isDeFiSecurity) {
          ...
          opinion = `[SOCIAL] Audit reputasi protokol. Somnex V3 record solid. Sentimen pool positif (@${sentiment}).`;
        } else {
      ...
      case "CUSTOM": // VANGUARD-X (Dynamic User Agent)
        const prompt = rawData.customPrompt || "Analytical Sniper";
        let multiplier = 1;
